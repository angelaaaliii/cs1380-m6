...
test.only('(15 pts) add support for iterative map-reduce', (done) => {
  ...
  for (const url of urls) {
    try {
      const found = execSync(`grep -q "${url}" "visited.txt"`, {encoding: 'utf-8'});
      counter++;
    } catch (e) {
      let rawPgContent;
      let pageText;
      let urls2;
      try {
        rawPgContent = execSync(`curl -skL --compressed "${url}"`, {encoding: 'utf-8'}).toString().trim();
      } catch (e) {
        console.log("GET RAW PG CONTENT FAIL", url);
        done(e);
      }

      try {
        urls2 = execSync(`echo ${JSON.stringify(rawPgContent)} | ./non-distribution/c/getURLs.js "https://en.wikipedia.org"`, { encoding: 'utf-8' }).toString();
      }
      catch (e) {
        console.log("URLS FAILED = ", e);
        done(e);
      }

      for (const url2 of urls2) {
        try {
          const found = execSync(`grep -q "${url2}" "visited.txt"`, {encoding: 'utf-8'});
        } catch (e) {
      ...
      try {
        ...
        console.log("PAGE TEXT FAIL = ", e);
        done(e);
      }

      try {
        execSync(`echo "${url}" >> visited.txt`, {encoding: 'utf-8'});
      } catch (e) {
        console.log("WRITING TO VISITED = ", e);
        done(e);
      }
...
test('(15 pts) add support for iterative map-reduce', (done) => {
  ...
  const dataset = [
    ...
    // {"https://en.wikipedia.org/wiki/Josh_Schache": {"original_url": "https://en.wikipedia.org/wiki/Josh_Schache"}}
  ];
